...
LWRCore::LWRCore(): RobotInterface(true)
{
  ...
  // This comment may not be valid anymore/*Note, the above is the more general way to find the path, but ros/package library does not work when the program is run as sudo, therefor we use this ugly hard coded path..this will only work if the interface is started from the RobotToolKit base folder*/
  // Get the path to RobotToolKit folder using ros::package
  ...
  driverPath = path_rtkpkgtools + "/." + driverPath;
...
void LWRCore::ControlUpdate(){
  ...
  //       }
  //       // cout<<"commanded JointPosition from LWRInterface:"<<endl;
  //       // for(int i=0;i<LBR_MNJ;i++){
  //       //   cout<<jpc2[i]<<" ";
  //       // }
  //       // cout<<" "<<endl;
  ...
  //       // set the desired impedance paramters

  //       cnt++;
  //       if(cnt==REL_FREQ_IMPEDANCE_UPDATE){

  // 	cnt = 0;
  // 	tempC_v  = mLWRRobot->GetCartStiffness();

  //         for(int i=0;i<3; i++){
  // 	  cart_imp_params[i] =  tempC_v(i);
  // 	  cart_imp_params[3+i] = tempC_v(3+i);
  // 	}
  //         mFRI->SetCommandedCartStiffness(cart_imp_params);

  // 	tempC_v = mLWRRobot->GetCartDamping();
  //         for(int i=0;i<3; i++){
  // 	  cart_imp_params[i] =  tempC_v(i);
  // 	  cart_imp_params[3+i] = tempC_v(3+i);
  // 	}
...
void LWRCore::SensorsUpdate(){
  ...
  for(int i=0;i<LBR_MNJ;i++){
    ...
    thisMeasurement.JointTorques(i) = (REALTYPE)tempJT[i];
  }
...
void LWRCore::ConsoleUpdate(){
  ...
  case FastResearchInterface::JOINT_IMPEDANCE_CONTROL:
    ...
    else if (mRobot->GetControlMode() == Robot::CTRLMODE_GRAVITYCOMPENSATION)
...
bool LWRCore::SetControlMode(int desiredMode){
  ...
  else if(desiredMode == Robot::CTRLMODE_TORQUE){
    mFRI->SetKRLBoolValue(0,true);
    mNCConsole.Print("Waiting for script...");
    mNCConsole.Render();
    mFRI-> WaitForKRCTick();
